...
const BookingTable = () => {
  ...
  const fetchData = async () => {
    const apiUrl = `${baseURL}/${selectedSport}Court/${
  ...
  return (
    <>
      <div className="bg-slate-700 p-10 mt-12 mb-12 rounded-xl shadow-black shadow-md">
        <h2 className="text-center text-lime-300 font-bold text-3xl mb-5">
          Time Table for Booking
        </h2>
        ...
        <div className="bg-lime-300 w-fit p-2 rounded-md mb-5 hover:bg-lime-400 ">
          ...
          <select
            className="bg-transparent active:ring-transparent "
            value={selectedSport}
              ...
              bookingData[0].slots.map((slot) => (
                <tr key={slot.startTime} className="h-8">
                  ...
                  {bookingData.map((court) => (
                    <td
                      className={
                        court.slots.find((s) => s.startTime === slot.startTime)
                          .isBooked
                          ? bookedStyle
                          : availableStyle
                      }
        ...
        <Link to={"/booking"}>
          <button className="my-5 rounded-full w-[150px] text-lg font-semibold bg-lime-300 border-2 border-lime-300 hover:bg-lime-400 p-2">
            Get Start
          </button>
        </Link>
      </div>
